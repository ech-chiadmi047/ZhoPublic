  ...
  <div class="post-details-global">
    <div class="post-details-left">
      <div class="slideshow-container">
        ...
        <div class="mySlides">
          <img src="/images/company/{{$image->image}}">
          <form action="{{ route('company-image.destroy', $image->id)}}" method="post">
            @csrf
            @method('DELETE')
    ...
    <div class="post-details-right">
      ...
      <div class="profile">
        ...
        <div>
          ...
          <p class="published-on">{{ $company->created_at }}</p>
        </div>
      </div>
      <!--  -->
      <hr>
      <form action="{{ route('zho-company.update', $company->id) }}" enctype="multipart/form-data" method="POST">
        @method("put")
        @csrf
        <!-- description -->
        ...
        <div class="post-flex">
          ...
          <select name="location" class="edit">
            <option value="{{ $company->location }}" selected>{{ $company->location }}</option>
        ...
        <div class="post-flex">
          ...
          <input id="date" type="date" class="form-control @error('date') is-invalid @enderror" name="date"
            value="{{ $company->date }}" required autocomplete="date" autofocus class="edit">
        </div>
        <!--  -->
        <hr>
        <!-- price -->
        <div class="post-flex">
          ...
          <input id="price" type="number" class="form-control @error('price') is-invalid @enderror" name="price"
            value="{{ $company->price }}" required autocomplete="price" autofocus class="post-desc" class="edit">
        </div>
        <!--  -->
        <hr>
        <!-- type -->
        <div class="post-flex">
          <img src="/svg/{{ PostType()[$company->type]['icon'] }}.svg" class='icon-type' />
        ...
        <hr>
        <!-- participant -->
        <h2 style="display: block;color: #da2400;text-align: center;">Participant</h2>
        <div class="post-flex">
          <!-- participantDesc -->
          <textarea id="participantDesc" class="form-control @error('participantDesc') is-invalid @enderror" name="participantDesc" required autofocus>
          {{ $company->participantDesc }}
          </textarea>
          <!--  -->
        </div>
        <hr style="width: 94%;margin-left: 3% !important;">
        <div class="post-flex">
          <p class="post-desc">Age</p>
          <input id="participantAge" type="number" class="form-control @error('participantAge') is-invalid @enderror"
            name="participantAge" value="{{ $company->participantAge }}" required autocomplete="participantAge"
            autofocus class="post-desc" class="edit">
        </div>
        <hr style="width: 94%;margin-left: 3% !important;">
        <div class="post-flex">
          <p class="post-desc">Gender</p>
          <select name="participantGender" class="edit">
            <option value="{{ $company->participantGender }}" selected>{{ $company->participantGender }}</option>
        ...
        <div class="post-contact">
          <div>
            ...
            <input id="fb" type="text" class="form-control @error('fb') is-invalid @enderror" name="fb" required
              placeholder="Enter your facebook" value="{{ $company->fb }}">
          </div>
          <div>
            ...
            <input id="num" type="tel" class="form-control @error('num') is-invalid @enderror" name="num" required
              placeholder="Enter your phone number" value="{{ $company->num }}">
          </div>
          <div>
            ...
            <input id="insta" type="text" class="form-control @error('insta') is-invalid @enderror" name="insta"
              required placeholder="Enter your instagram" value="{{ $company->insta }}">
          </div>
        </div>
        <!--  -->
        <hr>
